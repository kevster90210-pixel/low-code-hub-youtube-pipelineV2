...
def vizard_publish_to_youtube(video_path: Path, title: str) -> str | None:
            """Upload video to Vizard.ai and publish to YouTube. Returns the Vizard video ID or None."""
    if not VIZARD_API_KEY or not VIZARD_SOCIAL_ID:
                    print("  ⚠️  VIZARD_API_KEY or VIZARD_SOCIAL_ID not set — skipping YouTube publish.")
                    return None

    headers = {"Authorization": f"Bearer {VIZARD_API_KEY}"}

    # Step 1: Upload the video file
    print(f"  📤  Uploading video to Vizard.ai …")
    ...
    if not video_id:
                    print(f"  ⚠️  Could not extract video ID from Vizard upload response.")
        return None
    ...
    print(f"  ✅  Published to YouTube successfully!")
    return video_id
...
async def run():
    ...
    # Use the item title as the notebook title if not overridden
    ...
    async with await NotebookLMClient.from_storage() as client:
        ...
        # 6. Generate Video Overview
        print("\n🎬  Generating Video Overview …")
        video_path = None
        try:
                            video_status = await client.artifacts.generate_video(nb_id)
                            print(f"  Task ID: {video_status.task_id} — waiting for completion …")
            ...
            print(f"  ⚠️  Video generation failed (skipping): {e}")

        # 7. Publish video to YouTube via Vizard.ai
        if video_path and video_path.exists():
                            print("\n📺  Publishing to YouTube via Vizard.ai …")
                            try:
                                                    vizard_publish_to_youtube(video_path, notebook_title)
except Exception as e:
                print(f"  ⚠️  Vizard publish failed (skipping): {e}")
else:
            print("\n⏭️  Skipping Vizard publish — no video file available.")
